...
var Rectangle = class {
   ...
   * @memberof Rectangle
   */
  constructor(width = 0, height = 0, x = 0, y = 0, rot = false, allowRotation = void 0) {
    this.oversized = false;
    this._rot = false;
    this._allowRotation = void 0;
    this._dirty = 0;
    this._width = width;
    this._height = height;
    this._x = x;
...
var MaxRectsBin = class _MaxRectsBin extends Bin {
  ...
  add(...args) {
    ...
    if (args.length === 1) {
      ...
      rect = args[0];
      let tag = rect.data && rect.data.tag ? rect.data.tag : rect.tag ? rect.tag : void 0;
  ...
  repack() {
    ...
    return unpacked.length > 0 ? unpacked : void 0;
  }
  ...
  clone() {
    let clonedBin = new _MaxRectsBin(this.maxWidth, this.maxHeight, this.padding, this.options);
    for (let rect of this.rects) {
      clonedBin.add(rect);
    }
    return clonedBin;
  ...
  place(rect) {
    ...
    let node;
    let allowRotation;
    if (rect.hasOwnProperty("_allowRotation") && rect.allowRotation !== void 0) {
...
var OversizedElementBin = class _OversizedElementBin extends Bin {
  constructor(...args) {
    super();
    this.rects = [];
    if (args.length === 1) {
  ...
  clone() {
    let clonedBin = new _OversizedElementBin(this.rects[0]);
    return clonedBin;
  }
};
var EDGE_MAX_VALUE = 4096;
var PACKING_LOGIC;
(function(PACKING_LOGIC2) {
  PACKING_LOGIC2[PACKING_LOGIC2["MAX_AREA"] = 0] = "MAX_AREA";
  PACKING_LOGIC2[PACKING_LOGIC2["MAX_EDGE"] = 1] = "MAX_EDGE";
})(PACKING_LOGIC || (PACKING_LOGIC = {}));
var MaxRectsPacker = class {
  /**
  ...
  add(...args) {
    if (args.length === 1) {
      ...
      if (rect.width > this.width || rect.height > this.height) {
        ...
        let added = this.bins.slice(this._currentBinIndex).find((bin) => bin.add(rect) !== void 0);
        if (!added) {
  ...
  addArray(rects) {
    if (!this.options.tag || this.options.exclusiveTag) {
      ...
      if (rects.length === 0)
        return;
      rects.sort((a, b) => {
        const aTag = a.data && a.data.tag ? a.data.tag : a.tag ? a.tag : void 0;
        const bTag = b.data && b.data.tag ? b.data.tag : b.tag ? b.tag : void 0;
        return bTag === void 0 ? -1 : aTag === void 0 ? 1 : bTag > aTag ? -1 : 1;
      });
      let currentTag;
      let currentIdx = 0;
      let targetBin = this.bins.slice(this._currentBinIndex).find((bin, binIndex) => {
        ...
        for (let i = currentIdx; i < rects.length; i++) {
          const rect = rects[i];
          const tag = rect.data && rect.data.tag ? rect.data.tag : rect.tag ? rect.tag : void 0;
          if (i === 0)
            currentTag = tag;
          if (tag !== currentTag) {
            currentTag = tag;
